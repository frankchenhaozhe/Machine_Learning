...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    A = np.max(scores, axis=1) # N*1                       # shift the input(score) to zero to aviod 
    F = np.exp(scores - A.reshape(N, 1))  # N*C            # out of numerical range of floating point number
    P = F / np.sum(F, axis=1).reshape(N, 1)  # N*C         # compute the softmax 
    ...
    db2 = np.sum(dscore, axis=0)/N    # partial derivation of loss wrt. b2
    ...
    db1 = np.sum(dz1, axis = 0)/N  # [PLEASE IMPLEMENT]
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################

    grads['W2'] = dW2 + reg*W2        # don't forget reguarlization
    grads['b2'] = db2
    grads['W1'] = dW1 + reg*W1
    grads['b1'] = db1
    return loss, grads
  ...
  def predict(self, X):
    ...
    z2 = self.loss(X)
    softmax = np.exp(z2) / np.sum(np.exp(z2), axis = 1).reshape(-1, 1)
